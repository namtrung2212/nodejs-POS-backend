...
ex.quickCreate = async function (bus, params) {
    ...
    await ex.Base.setValue(obj, ex.tablename, "vendor_acc", params.vendor_acc);
    await ex.Base.setValue(obj, ex.tablename, "islanded", params.islanded);
...
ex.update = async function (params) {
    ...
    await sql.trySet("vendor_acc", params.vendor_acc);
    await sql.trySet("islanded", params.islanded);
...
ex.getone = async function (params) {
    ...
    sql = sql.field("email");
    sql = sql.field("islanded");

    await sql.tryField("vendor_acc", "hex(vendor_acc)");
